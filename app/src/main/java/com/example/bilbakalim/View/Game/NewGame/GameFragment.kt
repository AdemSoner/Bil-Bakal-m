...
class GameFragment : Fragment() {
    ...
    private fun initiliazeUI() {
        binding.readyBtn.setOnClickListener {
            ...
            setWord(viewModel.nextWord(),requireContext())
            startFlag = 1
        ...
        binding.pauseBtn.setOnClickListener {
            ...
            alert.setTitle("Oyun duraklatıldı.Devam etmek istiyor musunuz?")
                ...
                }).setMessage("Kalan süreniz= $pauseTime").create().show()
        }
        binding.trueBtn.setOnClickListener {
            ...
            if (viewModel.team1Score != gameFinishScore && viewModel.team2Score != gameFinishScore) viewModel.correctMedia(
                requireContext()
            )
            setWord(viewModel.nextWord(),requireContext())
        }
    ...
    private fun observeLiveData() {
        ...
        viewModel.timer.observe(viewLifecycleOwner, Observer { time ->
            time?.let {
                ...
                if (startFlag == 1 && it > 0 && pauseFlag == 0) viewModel.startTimer()
        ...
        viewModel.score.observe(viewLifecycleOwner, Observer { score ->
            score?.let {
                viewModel.writeScoreToFirebase()
                if (it == viewModel.userSettings[2].toInt()) viewModel.finishGame.value = true
                else binding.scoreTV.text = it.toString()
        ...
        viewModel.finishGame.observe(viewLifecycleOwner, Observer { value ->
            value?.let {
                if (it) {
                    ...
                    pauseFlag = 1
                    if (viewModel.team1Score == viewModel.userSettings[2].toInt()) binding.noWordMessageTV.text =
                        "${viewModel.teams[0]} " + getString(R.string.winnerGame)
                    else binding.noWordMessageTV.text =
                        "${viewModel.teams[1]} " + getString(R.string.winnerGame)
    ...
    private fun setWord(word: Words,context: Context) {
        if (binding.keyTV.textSize.toInt()!=95)
            setTextSizes(32f)
        val spanString = SpannableString(word.key)
        spanString.setSpan(UnderlineSpan(), 0, spanString.length, 0)
        if(spanString.length>12)
            setTextSizes(24f)
    ...
    private fun setTextSizes(value:Float){
        binding.keyTV.setTextSize(TypedValue.COMPLEX_UNIT_SP, value)
    }
