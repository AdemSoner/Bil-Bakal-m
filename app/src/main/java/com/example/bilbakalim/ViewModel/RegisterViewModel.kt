...
class RegisterViewModel() : ViewModel() {
    ...
    fun signUpWithFirebase(userInformation: User, context: Context?) {
        ...
        try {
            firebaseAuth.createUserWithEmailAndPassword(
                    userInformation.userEmail, userInformation.userPassword
    ...
    private fun addDatabase(user: User, context: Context?) {
        try {
            ...
            firebaseDatabase.child("Users").child(userUID).setValue(user)
            firebaseDatabase.child("Users").child(userUID).child("UID").setValue(userUID)
