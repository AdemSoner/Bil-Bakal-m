...
class SettingsViewModel : ViewModel() {
    ...
    fun getUserSettings() {
        ...
        val query = FirebaseDatabase.getInstance().reference.child("Users")
            .child(firebaseAuth?.uid.toString()).child("userSettings").orderByKey()
    ...
    fun setNewSettings() {
        ...
        firebaseDatabase.child("Users").child(firebaseAuth?.uid.toString()).child("userSettings")
            .setValue(userSettings).addOnCompleteListener {
                if (it.isSuccessful) {
